...
from waves.services import WaveClimateService
...
class WaveClimateBatchProcessor:
    ...
    def run(
        ...
        if point_id_col not in weather.columns and "point_id" in weather.columns:
            weather[point_id_col] = weather["point_id"]
        if point_id_col not in fetch_df.columns and "point_id" in fetch_df.columns:
            fetch_df[point_id_col] = fetch_df["point_id"]
        ...
        for point_id, wdf in weather.groupby(point_id_col):
            if point_id not in point_geom or point_id not in point_normals:
                continue
            tdf = fetch_df[fetch_df[point_id_col] == point_id][["direction", "fetch_m"]].copy()
            if tdf.empty:
                continue
    ...
    def export(
        ...
        daily_gdf, summary_gdf = self.run(points_gdf, fetch_df, weather_df, normal_field=normal_field)
