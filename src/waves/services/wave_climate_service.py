...
from src.bathymetry import BathymetryService
...
from src.waves.nearshore import BathymetryProfileProvider, NearshoreWaveTransformer
...
class WaveClimateService:
    ...
    bathymetry_service: BathymetryService | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.shore_normal_deg is not None:
            self._shore_normal = float(self.shore_normal_deg)
        else:
            self._shore_normal = ShoreNormalEstimator.estimate(self.trace_df)
        ...
        from src.waves.nearshore import BreakingModel, RefractionModel, ShoalingModel
    ...
    def cwef_by_direction(self, daily: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        if daily is None:
            daily = self.calculate_daily()
        return WaveClimateStatistics.cwef_by_direction(daily)

    def cwef_by_sector(self, daily: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        if daily is None:
            daily = self.calculate_daily()
        return WaveClimateStatistics.cwef_by_sector(daily)

    def annual_extremes(self, daily: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        if daily is None:
            daily = self.calculate_daily()
        return WaveClimateStatistics.annual_extremes(daily)
